...
> Test baseline: 192 passing, 3 skipped (broken exam flow) | DB: SQLite (dev) / PostgreSQL (prod)
...
- [ ] Remove duplicate inner import at `evaluaciones/views.py:237` (`from cursos.models import InscripcionCurso`)
- [ ] Verify `InscripcionCurso` is already imported at module level (line 9 area) — if not, add it there
- [ ] Unskip the 3 tests in `evaluaciones/tests.py:313-323` (`test_tomar_evaluacion_requires_inscripcion`, `test_tomar_evaluacion_post_aprobado`, `test_tomar_evaluacion_post_reprobado`)
- [ ] Write new test: `test_tomar_evaluacion_creates_intento_on_pass`
- [ ] Write new test: `test_tomar_evaluacion_marks_inscripcion_completado_when_all_evals_passed`
...
- [ ] In `resultado_evaluacion` view (line 265-277), pass `respuestas_detalle` in context: a dict mapping `pregunta.pk → alternativa.pk` derived from the `IntentoEvaluacion` or stored alongside it
- [ ] **Design decision needed**: Currently `IntentoEvaluacion` does NOT store which alternatives were selected — only the final score and pass/fail. Two options:
...
- [ ] If Option A: Add `respuestas = models.JSONField(default=dict, blank=True)` to `IntentoEvaluacion`, create & run migration, update `tomar_evaluacion` to save `respuestas` dict when creating `IntentoEvaluacion`
- [ ] Rewrite template `resultado_evaluacion.html` lines 73-111: replace `request.POST.respuestas` with `intento.respuestas` context variable
- [ ] Fix typo on line 33: change `"No aprobbaste"` → `"No aprobaste"`
- [ ] Add test: `test_resultado_evaluacion_shows_answer_summary`
- [ ] Add test: `test_resultado_evaluacion_shows_correct_alternatives`
...
- [ ] **Decision**: The `responder_pregunta_htmx` endpoint (views.py:280-293) is dead code — no template calls it, and it expects JSON body which HTMX doesn't send by default
- [ ] **Recommended action**: Remove the endpoint entirely for now. It can be re-added properly in P3 (Assessment Enhancements) if real-time per-question feedback is desired
- [ ] Remove URL pattern from `evaluaciones/urls.py:13`
- [ ] Remove the view function from `evaluaciones/views.py:280-293`
- [ ] Remove unused `json` import if no longer needed elsewhere in views.py
- [ ] Add test: verify `responder_pregunta_htmx` URL returns 404 (confirms removal)
...
- [ ] Create `calendario/` app: `python manage.py startapp calendario`
- [ ] Add `'calendario'` to `INSTALLED_APPS` in `kimun/settings.py`
...
### 1.6 Calendar — iCal Export

- [ ] Add `icalendar` to project dependencies: `pip install icalendar`
- [ ] Create `calendario/ical.py` with `exportar_eventos_ical(usuario)` function:
  - Generate `Calendar` with all events for the user (own created + enrolled courses)
  - Include `DTSTART`, `DTEND`, `SUMMARY`, `DESCRIPTION`, `UID`
  - Return `.ics` content as bytes
- [ ] Wire `calendario_ical_export` view to serve `.ics` file download
- [ ] Add "Exportar iCal" button to calendar template
- [ ] Write test: `test_ical_export_returns_valid_ics`

**Files to create:**
- `calendario/ical.py`

**Files to modify:**
- `calendario/views.py` — iCal export view
- `templates/calendario/calendario.html` — export button

**Dependencies:** P1.3

**Acceptance Criteria:** Clicking export downloads a valid `.ics` file. File imports correctly into Google Calendar / Apple Calendar.

**Effort:** S (1-2 days)

---

### 1.7 Calendar — View Tests

- [ ] Create `calendario/tests.py` with:
  - `EventoCalendarioModelTests` (creation, str, ordering, default color)
  - `EventoCalendarioFormTests` (valid, invalid dates)
  - `CalendarioViewTests` (login required, RBAC, month view renders, HTMX partial returns events)
  - `EventoCreateViewTests` (admin/docente can create, colaborador gets 403)
  - `EventoEditDeleteViewTests` (owner/admin can edit/delete, others get 403)
  - `IcalExportTests` (download returns .ics, contains expected events)
  - `SignalTests` (Curso save creates events, Evaluacion save creates events)

**Dependencies:** P1.1–P1.6

**Acceptance Criteria:** All calendar tests pass. Coverage of RBAC, views, forms, signals, iCal export.
...
| Commit | Description | Files |
|--------|-------------|-------|
| `feat(calendario): add EventoCalendario model and migration` | calendario app scaffold, models, settings |
| `feat(calendario): add auto-event generation signals` | signals.py, apps.py |
| `feat(calendario): add generar_eventos_calendario management command` | management command |
| `feat(calendario): add views, urls, and forms` | views, urls, forms |
| `feat(calendario): add calendar templates` | all template files |
| `feat(calendario): add iCal export` | ical.py, view update, template button |
| `test(calendario): add comprehensive tests` | tests.py |
| `feat(calendario): add calendar nav link to base.html` | base.html |

---

## Priority 2: Assignment Submissions

### 2.1 Assignment — Model & Migrations

- [ ] Create `tareas/` app: `python manage.py startapp tareas`
- [ ] Add `'tareas'` to `INSTALLED_APPS`
- [ ] Create models:
  ```python
  ...
  ```
- [ ] Run `makemigrations tareas` and `migrate`
- [ ] Write model tests: creation, `__str__`, `unique_together` constraint, ordering

**Files to create:** `tareas/__init__.py`, `tareas/models.py`, `tareas/apps.py`, `tareas/admin.py`
...
### 2.2 Assignment — Views & URL Routing

- [ ] Create `tareas/views.py` with function-based views:
  - `tarea_list(request, curso_pk)` — list assignments for a course
  - `tarea_detail(request, pk)` — assignment detail + submission status
  - `tarea_create(request, curso_pk)` — admin/docente only
  - `tarea_edit(request, pk)` — admin/docente only
  - `tarea_delete(request, pk)` — admin/docente only
  - `entrega_create(request, tarea_pk)` — colaborador submits file
  - `entrega_grade(request, pk)` — docente/admin grades submission
- [ ] Create `tareas/urls.py` with `app_name = 'tareas'`
- [ ] Include in `kimun/urls.py`
- [ ] Apply RBAC:
  - `tarea_create/edit/delete`: `@docente_or_admin_required` + `@course_owner_or_admin`
  - `entrega_create`: `@login_required` (colaborador enrolled in course)
  - `entrega_grade`: `@docente_or_admin_required`
- [ ] Add nav link to course detail page for "Tareas"

**Files to create:** `tareas/views.py`, `tareas/urls.py`

**Files to modify:** `kimun/urls.py`

**Dependencies:** P2.1

**Acceptance Criteria:** URL routing works. RBAC enforced: colaborador can submit, docente/admin can create and grade.

**Effort:** M (3-4 days)

---

### 2.3 Assignment — Forms

- [ ] Create `tareas/forms.py`:
  - `TareaForm(ModelForm)` — titulo, descripcion, fecha_limite, puntaje_maximo
  - `EntregaTareaForm(Form)` — archivo, comentario (not ModelForm — handle creation in view)
  - `CalificacionForm(Form)` — puntaje_obtenido, retroalimentacion
  - Validation: puntaje_obtenido <= tarea.puntaje_maximo, fecha_limite not in past for create
  - Widget attrs: `input-field` class

**Files to create:** `tareas/forms.py`

**Dependencies:** P2.1

**Acceptance Criteria:** Forms validate correctly, follow existing patterns.

**Effort:** S (1 day)

---

### 2.4 Assignment — Templates

- [ ] Create templates:
  - `templates/tareas/tarea_list.html` — list of assignments for a course
  - `templates/tareas/tarea_detail.html` — assignment detail with submission status
  - `templates/tareas/tarea_form.html` — create/edit assignment form
  - `templates/tareas/tarea_confirm_delete.html` — delete confirmation
  - `templates/tareas/entrega_form.html` — file upload submission form
  - `templates/tareas/entrega_grade.html` — grading form for docente
- [ ] Add "Tareas" section to course detail template

**Files to create:** 6 template files in `templates/tareas/`

**Files to modify:** `templates/cursos/curso_detail.html` — add tasks section

**Dependencies:** P2.2, P2.3

**Acceptance Criteria:** Full CRUD flow works. File upload and download work. Grading flow works. Mobile-responsive.

**Effort:** M (3-4 days)

---

### 2.5 Assignment — Auto-Generate Calendar Events

- [ ] Add signal in `calendario/signals.py`: `post_save` on `Tarea` → create `clase_deadline` type EventoCalendario
- [ ] Ensure task deadline appears on student calendar
- [ ] Write test: creating a Tarea creates a calendar event

**Files to modify:** `calendario/signals.py`

**Dependencies:** P1.2, P2.1

**Acceptance Criteria:** Assignment deadlines appear on calendar automatically.
...
- [ ] Create `tareas/tests.py`:
  - `TareaModelTests`
  - `EntregaTareaModelTests`
  - `TareaFormTests`, `CalificacionFormTests`
  - `TareaListViewTests` (RBAC, enrollment check)
  - `TareaCreateViewTests` (admin/docente only, course owner check)
  - `EntregaCreateViewTests` (colaborador can submit, file upload, deadline enforcement)
  - `EntregaGradeViewTests` (docente/admin can grade, puntaje validation)

**Dependencies:** P2.1–P2.4

**Acceptance Criteria:** All assignment tests pass. RBAC covered for all views.
...
| Commit | Description | Files |
|--------|-------------|-------|
| `feat(tareas): add Tarea and EntregaTarea models` | tareas app scaffold, models, settings |
| `feat(tareas): add views, urls, and forms` | views, urls, forms |
| `feat(tareas): add assignment templates` | all template files |
| `feat(tareas): integrate task deadlines with calendar signals` | calendario/signals.py |
| `test(tareas): add comprehensive tests` | tests.py |
...
## Priority 5: Analytics & Reporting Enhancements

### 5.1 Enhanced Dashboard Charts

- [ ] Add chart library: **Chart.js via CDN** (lightweight, no build step, matches Tailwind CDN approach)
- [ ] Create `templates/reportes/partials/` directory for reusable chart partials
- [ ] Add charts to `reportes/dashboard`:
  - Course completion rate (donut chart)
  - Enrollment over time (line chart)
  - Evaluation pass/fail rates (bar chart)
  - At-risk students table
- [ ] Use HTMX partial rendering for chart data endpoints
- [ ] All text in Spanish
...
**Acceptance Criteria:** Dashboard loads with interactive charts. Data reflects current state.
...
- [ ] Add `get_at_risk_students()` method or management command:
  - Criteria: enrolled but no progress in 7+ days, approaching deadline without completions, failed all evaluation attempts
  - Returns queryset of `Usuario` with risk reasons
- [ ] Add to dashboard: table/cards showing at-risk students with risk reason
- [ ] Create `reportes/management/commands/identificar_riesgo.py` for cron scheduling
- [ ] Write tests

**Files to modify:** `reportes/views.py`, `templates/reportes/dashboard.html`

**Files to create:** `reportes/management/commands/identificar_riesgo.py`

**Dependencies:** P0 complete

**Acceptance Criteria:** Dashboard shows at-risk students. Management command outputs list.
...
- [ ] Add per-course progress view with heatmap visualization:
  - Rows: students, Columns: course components (clases + evaluaciones)
  - Cell color: green (complete), yellow (in progress), red (overdue), gray (not started)
  - Filter by course
- [ ] HTMX-based filtering
- [ ] Admin/docente only

**Dependencies:** P5.1

**Acceptance Criteria:** Heatmap renders correctly with color-coded progress. Filter works.
